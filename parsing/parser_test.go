...
func TestParser_ParseStatement(t *testing.T) {
	var tests = []struct {
		...
		{
			s: `INSERT INTO tbl (name, email) VALUES (demo, demo)`,
			stmt: &ps.InsertStatement{
				Cols:      []string{"name", "email"},
				Values:    []string{"demo", "demo"},
				TableName: "tbl",
			},
		},
	...
	for i, tt := range tests {
		stmt, err := ps.NewParser(strings.NewReader(tt.s)).Parse()
